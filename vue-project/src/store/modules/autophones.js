...
const actions = {
  ...
  updateItem({ commit, state }, { updatedItem }) {
    ...
    const autophoneIndex = state.autophone.findIndex(
      (item) =>
        item.caller === state.editedItem.caller &&
        item.date == state.editedItem.date &&
        item.timeStart == state.editedItem.timeStart
    )
